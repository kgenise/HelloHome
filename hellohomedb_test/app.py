...
def submit():
  if request.method =='POST':
    if 'agent_form' in request.form:
      ...
      agent = Agent(agent_fname, agent_lname, company, agent_phone, agent_email)
      ...
      buyer = Buyer(buyer_fname, buyer_lname, buyer_email)
      db.session.add(buyer)
      db.session.commit()

      # Fetch certain buyer
      buyerResult=db.session.query(Buyer).filter(Buyer.buyer_id==1)
      for result in buyerResult:
